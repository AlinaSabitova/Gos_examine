...
| **Скорость поступления** | **до 1 000 транзакций/с** (пиковая) | Необходима высокопроизводительная шина с низкой латентностью (Kafka) и поточная обработка сложных событий (Flink / Spark Streaming) |
...
| **Обработка** | Антифрод в реал-тайм (<1 с) + Оценка рисков (batch) | **Kappa-архитектура** (предпочтительно) или Lambda с жестким SLA. Стриминг обязателен. |
| **Доступность** | **99.999%** (Five Nines, ~5.26 мин простоя в год) | Гео-резервирование, Multi-AZ кластер, Active-Active или Hot-Standby ЦОД, полное резервирование NameNode и брокеров Kafka |
| **Время отклика** | **< 1 секунды** (на принятие решения по фроду) | In-memory вычисления, K-V хранилище с низкой латентностью (Redis/Aerospike), процессор правил (Drools) или быстрый ML-инференс |
